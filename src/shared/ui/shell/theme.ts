...
/**
 * The MUI theme for an app on this kit.
 *
 * The CSS variables in tokens.css are the source of truth for everything the
 * kit's own components draw. This covers what MUI does not read from CSS
 * variables — its palette, its typography scale, and the parts (buttons,
 * fields, dialogs, tooltips, tables, scrims) an app uses directly. The values
 * are the same ones; where MUI needs a literal, the literal matches the token.
 *
 * Applied through a nested ThemeProvider by `ThemeScope`, so apps that predate
 * the kit keep the platform-wide theme untouched.
 */
export function createPlatformTheme(mode: ThemeMode) {
  ...
  return createTheme({
    ...
    components: {
      ...
      MuiTextField: {
        // Data screens are dense; small is the default so no screen has to say so.
      ...
      MuiDialogContent: { styleOverrides: { root: { padding: '4px 24px', fontSize: '14px' } } },
      MuiDialogActions: { styleOverrides: { root: { padding: '16px 24px 20px', gap: '8px' } } },
      MuiBackdrop: {
        styleOverrides: {
