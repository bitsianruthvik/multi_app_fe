...
import { Box, Checkbox, IconButton, Menu, MenuItem, ListItemText, Tooltip, Button } from '@mui/material';
...
import { readPref, readRawPref, writePref, writeRawPref } from './storage';

/**
 * The one table (DESIGN_SYSTEM.md §4.2 / §7).
 *
 * Without it, every screen hand-rolls `<TableContainer><Table>` with its own
 * header styling, its own sort (or none), no column control, no density, no
 * selection and no export. That inconsistency is the main reason an app reads
 * as "a theme over a spreadsheet". Everything that wants a multi-column grid
 * uses this.
 *
 * Deliberately NOT virtualized. Windowing a semantic `<table>` costs correct
 * sticky headers, correct column widths and keyboard row navigation.
 * Client-side pagination is cheaper and is what people actually expect — they
 * want to know they're on page 3 of 7, not scroll a rope of unknown length. A
 * screen that genuinely needs tens of thousands of rows wants a purpose-built
 * virtualized grid, not this.
 *
 * Row height, padding and font size come from --row-h / --row-px / --row-fs, so
 * the density control retunes every table at once via data-density on <html>.
 * Nothing here hardcodes a row height, a colour or a shadow.
 */
...
export interface DataColumn<T> {
  ...
  numeric?: boolean;
  width?: number | string;
  /** Start hidden; the user can enable it from the column menu. */
...
/**
 * The table's card — or a plain box when it already sits inside one
 * (`bare`, for a table in a SectionCard or a detail tab).
 */
...
/**
 * Density is global — one attribute on <html>, one stored key, so every table
 * in the app agrees and the choice survives a reload. It is deliberately NOT
 * part of a table's per-screen `storageKey` prefs: someone who wants compact
 * rows wants them everywhere, not to re-set it on each screen.
 */
function readDensity(): Density {
  const raw = readRawPref('density');
...
function applyDensity(d: Density) {
  ...
  writeRawPref('density', d);
}

function readTablePref<V>(key: string | undefined, suffix: string, fallback: V): V {
  if (!key) return fallback;
  return readPref(`table:${key}:${suffix}`, fallback);
}

function writeTablePref(key: string | undefined, suffix: string, value: unknown) {
  if (!key) return;
  writePref(`table:${key}:${suffix}`, value);
}
...
export function DataTable<T>({
  ...
  /** No card of its own and no column toolbar — for a table already in a card. */
  bare?: boolean;
}) {
  ...
  const [pageSize, setPageSize] = useState<number>(() =>
    readTablePref(storageKey, 'pageSize', initialPageSize),
  );
  ...
  useEffect(() => {
    writeTablePref(storageKey, 'hidden', hidden);
  }, [storageKey, hidden]);
  useEffect(() => {
    writeTablePref(storageKey, 'pageSize', pageSize);
  }, [storageKey, pageSize]);
  useEffect(() => {
    applyDensity(density);
  }, [density]);

  // A filter change upstream shrinks `rows`; without this the reader can be
  // stranded on a page that no longer exists and see an empty table.
  useEffect(() => {
    setPage(0);
  }, [rows.length, sortKey, sortDir, pageSize]);
  ...
  const onBodyKeyDown = (e: React.KeyboardEvent) => {
    ...
    const nextIdx =
      e.key === 'ArrowDown' ? Math.min(idx + 1, cells.length - 1) : Math.max(idx - 1, 0);
    e.preventDefault();
    cells[nextIdx]?.focus();
  };
  ...
  const hasToolbar =
    !!storageKey || !!exportName || (!bare && columns.some((c) => !c.alwaysVisible));
  ...
  return (
    <Box>
      ...
      {hasToolbar && (
        <Box
          sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 1, justifyContent: 'flex-end' }}
        >
          <Box
            sx={{
              mr: 'auto',
          ...
          <Tooltip title="Row density">
            <IconButton
              size="small"
              onClick={(e) => setDensityMenu(e.currentTarget)}
      ...
      <Frame bare={bare}>
        <Box sx={{ overflowX: 'auto', maxHeight, overflowY: maxHeight ? 'auto' : undefined }}>
          ...
          >
            <Box
              component="thead"
              sx={{ position: 'sticky', top: 0, zIndex: 1, background: 'var(--c-surface-2)' }}
            >
              <Box component="tr">
                {selectable && (
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          '&:hover': {
                            background: isSel ? 'var(--c-primary-50)' : 'var(--c-surface-2)',
                          },
                          '&:hover .dt-actions, &:focus-within .dt-actions': {
                            opacity: 1,
                            pointerEvents: 'auto',
                          },
                        }}
                          ...
                          >
                            <Box
                              ...
                              sx={{
                                ...
                                // pointer to hover with. On a tablet there is no hover, and a
                                // button nobody can see is a button nobody has.
        ...
        {!loading && pageCount > 1 && (
          ...
          >
            <Box
              sx={{ fontSize: 12, color: 'var(--c-text-2)', fontFamily: 'var(--font-mono)' }}
            >
