...
import type {
  FabPlant,
  FabResource,
  FabResourceCustomField,
  FabResourceType,
  FabResourceTypeProperty,
  FabStockLocation,
...
// ── standard field definitions (SAP Work Center) ─────────────────────────────
const STD_FIELDS = [
  // Capacity
  ...
  { key: 'schedulingBasis',   label: 'Scheduling Basis',      unit: '',    inputType: 'select',
    options: ['machine', 'labor'], section: 'Scheduling' },
  // Costing
...
// ── StdFieldsSection ─────────────────────────────────────────────────────────
function StdFieldsSection({
  draft, onChange, typeDefaults, canEdit,
}: {
  draft: StdDraft;
  onChange: (k: StdKey, v: string) => void;
  typeDefaults?: StdDraft;  // for resource-level: shows inherited column
  canEdit: boolean;
}) {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {sections.map((section) => (
        <Box key={section}>
          <Typography variant="subtitle2" color="primary" sx={{ mb: 1, fontWeight: 700 }}>{section}</Typography>
          <Table size="small">
            <TableHead>
              <TableRow sx={{ '& th': { fontWeight: 700, bgcolor: 'grey.50' } }}>
                <TableCell>Field</TableCell>
                {typeDefaults && <TableCell>Type Default</TableCell>}
                <TableCell>{typeDefaults ? 'Resource Override' : 'Default Value'}</TableCell>
                {typeDefaults && <TableCell>Effective</TableCell>}
            ...
            <TableBody>
              {STD_FIELDS.filter((f) => f.section === section).map((f) => {
                ...
                const isOverriding = typeDefaults && overridden !== '';
                    ...
                    {typeDefaults && (
                      <TableCell>
                        <Typography variant="body2" color="text.secondary">
                          {inherited !== '' ? inherited : <span style={{ color: '#bbb' }}>—</span>}
                        </Typography>
                      </TableCell>
                    )}
                    <TableCell>
                      {!canEdit ? (
                        <Typography variant="body2">{overridden || <span style={{ color: '#bbb' }}>—</span>}</Typography>
                      ) : f.inputType === 'select' ? (
                        <TextField
                          select size="small"
                          value={overridden}
                          onChange={(e) => onChange(f.key as StdKey, e.target.value)}
                          sx={{ minWidth: 120 }}
                        >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          <TextField
                            ...
                            value={overridden}
                            onChange={(e) => onChange(f.key as StdKey, e.target.value)}
                            placeholder={typeDefaults ? (inherited || 'inherit') : ''}
                            sx={{ width: 150 }}
                          />
                          {typeDefaults && overridden !== '' && (
                            <Tooltip title="Clear override (revert to type default)">
                              <IconButton size="small" onClick={() => onChange(f.key as StdKey, '')}>
                    ...
                    {typeDefaults && (
                      <TableCell>
                        <Chip
                          ...
                          color={isOverriding ? 'primary' : 'default'}
                          variant={isOverriding ? 'filled' : 'outlined'}
                        />
                      </TableCell>
                    )}
                    <TableCell>
                      <Typography variant="body2" color="text.secondary">{f.unit || '—'}</Typography>
                    </TableCell>
                  </TableRow>
                );
              })}
...
function CustomFieldsEditor({
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fabQuery<QueryResult<FabResourceCustomField>>('fabErpResourceCustomField', {
  ...
  useEffect(() => { load(); }, [level, levelId]);

  async function saveField(draft: CfDraft, editId?: number) {
    if (!draft.fieldKey.trim() || !draft.fieldLabel.trim()) {
      setCfErr('Key and Label are required.'); return;
    }
    ...
    try {
      const payload = {
        ...
        field_label: draft.fieldLabel.trim(),
        field_type:  draft.fieldType,
        field_value: draft.fieldValue || null,
        sort_order:  editId ? (fields.find((f) => f.id === editId)?.sortOrder ?? fields.length) : fields.length,
      };
      if (editId) {
        await fabMutate('fabErpResourceCustomField', 'update', { id: editId, ...payload });
      } else {
        await fabMutate('fabErpResourceCustomField', 'insert', payload);
      }
    ...
    <Box>
      {loading ? (
        ...
        <>
          ...
          {fields.length === 0 ? (
            <Typography variant="body2" color="text.secondary">
              No custom fields defined. {canManage ? 'Click "Add Custom Field" to add up to 10.' : ''}
            </Typography>
          ) : (
            <Table size="small">
              <TableHead>
              ...
              <TableBody>
                {fields.map((f) => (
                  <TableRow key={f.id} hover>
                    ...
                    <TableCell>{f.fieldValue ?? <span style={{ color: '#bbb' }}>—</span>}</TableCell>
                    {canManage && (
                      <TableCell>
                        <Stack direction="row" spacing={0.5}>
                          <Tooltip title="Edit">
                            <IconButton size="small" onClick={() => {
                              ...
                              setEditDlg({
                                id: f.id,
                                draft: {
                                  fieldKey: f.fieldKey, fieldLabel: f.fieldLabel,
                                  fieldType: f.fieldType, fieldValue: f.fieldValue ?? '',
                                },
                              });
                            }}>
                          ...
                          <Tooltip title="Delete">
                            <IconButton size="small" color="error" onClick={() => setDelTarget(f)}>
      ...
      <Dialog open={!!currentDlg} onClose={() => { setAddDlg(null); setEditDlg(null); }} maxWidth="sm" fullWidth>
        <DialogTitle>{currentDlgId ? 'Edit Custom Field' : 'Add Custom Field'}</DialogTitle>
        <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
          {cfErr && <Alert severity="error">{cfErr}</Alert>}
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Field Key" size="small" fullWidth required
                value={currentDlg?.fieldKey ?? ''}
                onChange={(e) => setCurrentDlgField('fieldKey', e.target.value.replace(/\s+/g, '_').toLowerCase())}
                disabled={!!currentDlgId}
                helperText="Unique identifier, e.g. material_grade"
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Label" size="small" fullWidth required
                value={currentDlg?.fieldLabel ?? ''}
                onChange={(e) => setCurrentDlgField('fieldLabel', e.target.value)}
                helperText="Display name for this field"
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField select label="Type" size="small" fullWidth
                value={currentDlg?.fieldType ?? 'text'}
                onChange={(e) => setCurrentDlgField('fieldType', e.target.value)}
              >
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Default Value" size="small" fullWidth
                value={currentDlg?.fieldValue ?? ''}
                onChange={(e) => setCurrentDlgField('fieldValue', e.target.value)}
        ...
        <DialogActions>
          ...
          <Button variant="contained" disabled={cfSaving} onClick={() => currentDlg && saveField(currentDlg, currentDlgId)}>
            {cfSaving ? <CircularProgress size={16} /> : 'Save'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={!!delTarget} onClose={() => setDelTarget(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>Delete <strong>{delTarget?.fieldLabel}</strong> ({delTarget?.fieldKey})? This cannot be undone.</Typography>
        </DialogContent>
        <DialogActions>
...
// ── Formula Properties Editor (extracted from inline panel) ───────────────────
interface PropertyDraft {
  property_key: string; property_label: string; unit: string; default_value: string;
}
...
function FormulaPropertiesEditor({ rtId, canManage }: { rtId: number; canManage: boolean }) {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fabQuery<QueryResult<FabResourceTypeProperty>>('fabErpResourceTypeProperty', {
        filters: { resourceTypeId: rtId },
        orderBy: [{ field: 'propertyKey', direction: 'asc' }],
  ...
  async function saveProp() {
    if (!propDraft.property_key.trim() || !propDraft.property_label.trim()) {
      setPropErr('Key and Label are required.'); return;
    }
    setPropSaving(true); setPropErr('');
    try {
      const payload = {
        resource_type_id: rtId,
        property_key:     propDraft.property_key.trim(),
        property_label:   propDraft.property_label.trim(),
        unit:             propDraft.unit.trim() || null,
        default_value:    propDraft.default_value !== '' ? Number(propDraft.default_value) : null,
      };
      if (propDlg.item) {
        await fabMutate('fabErpResourceTypeProperty', 'update', { id: propDlg.item.id, ...payload });
      } else {
        await fabMutate('fabErpResourceTypeProperty', 'insert', payload);
      }
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mb: 1 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Formula properties define <code>machine.*</code> variables available in routing formulas.
          E.g. <code>machine.speed</code>, <code>machine.feed_rate</code>.
        </Typography>
      </Box>
        ...
        <>
          {canManage && (
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 1 }}>
              <Button size="small" startIcon={<AddIcon />} variant="outlined"
                onClick={() => { setPropDraft(BLANK_PROP()); setPropErr(''); setPropDlg({ open: true, item: null }); }}>
                Add Property
              </Button>
            </Box>
          )}
          ...
          {props.length === 0 ? (
            <Typography variant="body2" color="text.secondary">No formula properties defined yet.</Typography>
          ) : (
            <Table size="small">
              <TableHead>
                <TableRow sx={{ '& th': { fontWeight: 700 } }}>
                  <TableCell>Key (formula variable)</TableCell>
                  <TableCell>Label</TableCell>
              ...
              <TableBody>
                {props.map((p) => (
                  <TableRow key={p.id} hover>
                    <TableCell><code>machine.{p.propertyKey}</code></TableCell>
                          ...
                          <Tooltip title="Edit">
                            <IconButton size="small" onClick={() => {
                              setPropDraft({
                                ...
                                unit:           p.unit ?? '',
                                default_value:  p.defaultValue != null ? String(p.defaultValue) : '',
                              });
                              setPropErr('');
                              setPropDlg({ open: true, item: p });
                            }}>
                              <EditIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <IconButton size="small" color="error" onClick={() => setDelTarget(p)}>
                              <DeleteIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
                        </Stack>
      ...
      <Dialog open={propDlg.open} onClose={() => setPropDlg({ open: false, item: null })} maxWidth="sm" fullWidth>
        <DialogTitle>{propDlg.item ? `Edit — ${propDlg.item.propertyKey}` : 'Add Formula Property'}</DialogTitle>
        <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
          {propErr && <Alert severity="error">{propErr}</Alert>}
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Property Key" size="small" fullWidth required
                value={propDraft.property_key}
                onChange={(e) => setPropDraft((d) => ({ ...d, property_key: e.target.value.replace(/\s/g, '_') }))}
                disabled={!!propDlg.item}
                helperText='e.g. "speed" → machine.speed in formulas'
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Label" size="small" fullWidth required
                value={propDraft.property_label}
                onChange={(e) => setPropDraft((d) => ({ ...d, property_label: e.target.value }))}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Unit" size="small" fullWidth
                value={propDraft.unit}
                onChange={(e) => setPropDraft((d) => ({ ...d, unit: e.target.value }))}
                placeholder="e.g. mm/min"
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Default Value" type="number" size="small" fullWidth
                value={propDraft.default_value}
                onChange={(e) => setPropDraft((d) => ({ ...d, default_value: e.target.value }))}
              />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" disabled={propSaving} onClick={saveProp}>
            {propSaving ? <CircularProgress size={16} /> : 'Save'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={!!delTarget} onClose={() => setDelTarget(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>Delete <strong>{delTarget?.propertyKey}</strong>? This cannot be undone.</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDelTarget(null)}>Cancel</Button>
          <Button color="error" variant="contained" onClick={async () => {
            if (!delTarget) return;
            await fabMutate('fabErpResourceTypeProperty', 'delete', { id: delTarget.id });
            setDelTarget(null); await load();
          }}>Delete</Button>
        </DialogActions>
      </Dialog>
...
function ResourceTypeDetailDialog({
  open, initial, plants, canManage, onClose, onSaved,
}: {
  open: boolean; initial: FabResourceType | null;
  plants: FabPlant[]; canManage: boolean;
  ...
  useEffect(() => {
    ...
    setTab(0); setErr('');
    if (initial) {
      setBasic({ name: initial.name, code: initial.code, category: initial.category ?? '', plantId: initial.plantId ?? null });
      setStd(fromRtStd(initial));
    } else {
      setBasic({ name: '', code: '', category: '', plantId: null });
      setStd(blankStd());
    }
  }, [open, initial]);
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        category:  basic.category.trim() || null,
        plant_id:  basic.plantId,
        ...toStdPayload(std),
      };
      if (isNew) {
        await fabMutate('fabErpResourceType', 'insert', payload);
      } else {
        await fabMutate('fabErpResourceType', 'update', { id: initial!.id, ...payload });
      }
      onSaved();
      if (isNew) onClose();
    } catch (e: unknown) {
  ...
  const tabLabels = isNew
    ...
    : ['Basic Info', 'Capacity & Scheduling', 'Custom Fields (10)', 'Formula Properties'];

  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      <DialogTitle>
        {isNew ? 'New Resource Type' : `Resource Type — ${initial?.code} · ${initial?.name}`}
      </DialogTitle>
      ...
      <DialogContent dividers sx={{ minHeight: 400 }}>
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Code *" size="small" fullWidth
                value={basic.code} onChange={(e) => setBasic((d) => ({ ...d, code: e.target.value }))}
                disabled={!canManage}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Name *" size="small" fullWidth
                value={basic.name} onChange={(e) => setBasic((d) => ({ ...d, name: e.target.value }))}
                disabled={!canManage}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField label="Category" size="small" fullWidth
                value={basic.category} onChange={(e) => setBasic((d) => ({ ...d, category: e.target.value }))}
                disabled={!canManage}
                helperText="e.g. Machine, Labor, Tool"
              />
            </Grid>
            <Grid size={{ xs: 12 }}>
              <TextField select label="Plant" size="small" fullWidth
                value={basic.plantId ?? ''}
                onChange={(e) => setBasic((d) => ({ ...d, plantId: e.target.value === '' ? null : Number(e.target.value) }))}
                helperText="Leave blank for company-wide"
                disabled={!canManage}
              >
        ...
        {/* Tab 1: Capacity & Scheduling */}
        {tab === 1 && (
          <StdFieldsSection draft={std} onChange={(k, v) => setStd((d) => ({ ...d, [k]: v }))} canEdit={canManage} />
        )}

        {/* Tab 2: Custom Fields (edit mode only) */}
        {tab === 2 && !isNew && initial && (
          <CustomFieldsEditor level="resource_type" levelId={initial.id} canManage={canManage} />
        )}

        {/* Tab 3: Formula Properties (edit mode only) */}
        {tab === 3 && !isNew && initial && (
          <FormulaPropertiesEditor rtId={initial.id} canManage={canManage} />
        )}
      </DialogContent>
      <DialogActions>
        ...
        {canManage && (tab === 0 || tab === 1) && (
          <Button variant="contained" onClick={save} disabled={saving}>
            {saving ? <CircularProgress size={16} /> : (isNew ? 'Create' : 'Save Changes')}
          </Button>
        )}
      </DialogActions>
    </Dialog>
...
function ResourceDetailDialog({
  open, initial, resourceTypes, plants, canManage, onClose, onSaved,
}: {
  open: boolean; initial: FabResource | null;
  resourceTypes: FabResourceType[]; plants: FabPlant[];
  ...
  useEffect(() => {
    ...
    setTab(0); setErr('');
    if (initial) {
      setBasic({ name: initial.name, code: initial.code, resourceTypeId: initial.resourceTypeId, plantId: initial.plantId ?? null, stockLocationId: initial.stockLocationId ?? null, shiftCalendarId: (initial as any).shiftCalendarId ?? null });
  ...
  useEffect(() => {
    ...
    fabQuery<QueryResult<FabStockLocation>>('fabErpStockLocation', {
      filters: { plantId: basic.plantId }, orderBy: [{ field: 'name', direction: 'asc' }], pagination: { limit: 500 },
    }).then((res) => setStockLocations(res.data ?? [])).catch(() => setStockLocations([]));
  }, [open, basic.plantId]);

  useEffect(() => {
    if (!open) return;
    fabQuery<QueryResult<FabShiftCalendarOption>>('fabErpShiftCalendar', {
  ...
  useEffect(() => {
    ...
    if (!stockLocations.some((l) => l.id === basic.stockLocationId)) {
      setBasic((d) => ({ ...d, stockLocationId: null }));
    }
  }, [stockLocations, basic.stockLocationId]);

  async function save() {
    if (!basic.name.trim() || !basic.code.trim() || !basic.resourceTypeId) {
      setErr('Name, Code and Resource Type are required.'); return;
    }
    setSaving(true); setErr('');
    try {
      const payload = {
        name:               basic.name.trim(),
        code:               basic.code.trim(),
        resource_type_id:   basic.resourceTypeId,
        plant_id:           basic.plantId,
        stock_location_id:  basic.stockLocationId,
        shift_calendar_id:  basic.shiftCalendarId,
        ...toStdPayload(std),
      };
      if (isNew) {
  ...
  const tabLabels = isNew
    ? ['Basic Info', 'Capacity & Scheduling']
    : ['Basic Info', 'Capacity & Scheduling', 'Custom Fields (10)'];

  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      <DialogTitle>
        {isNew ? 'New Resource' : `Resource — ${initial?.code} · ${initial?.name}`}
      </DialogTitle>
      <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 3 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)}>
          {tabLabels.map((l, i) => <Tab key={i} label={l} />)}
      ...
      <DialogContent dividers sx={{ minHeight: 400 }}>
        ...
        {tab === 0 && (
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12 }}>
              <TextField select label="Resource Type *" size="small" fullWidth
                value={basic.resourceTypeId ?? ''}
                onChange={(e) => setBasic((d) => ({ ...d, resourceTypeId: e.target.value === '' ? null : Number(e.target.value) }))}
                disabled={!canManage}
              >
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField select label="Stock Location" size="small" fullWidth
                value={basic.stockLocationId ?? ''}
                onChange={(e) => setBasic((d) => ({ ...d, stockLocationId: e.target.value === '' ? null : Number(e.target.value) }))}
                disabled={!canManage || basic.plantId == null}
                helperText={basic.plantId == null ? 'Select a plant first' : 'Optional'}
              >
            ...
            <Grid size={{ xs: 12 }}>
              <TextField select label="Shift Calendar" size="small" fullWidth
                value={basic.shiftCalendarId ?? ''}
                onChange={(e) => setBasic((d) => ({ ...d, shiftCalendarId: e.target.value === '' ? null : Number(e.target.value) }))}
                helperText="Working hours / shift pattern for this machine"
                disabled={!canManage}
              >
                <MenuItem value="">— (none)</MenuItem>
        ...
        {/* Tab 1: Capacity & Scheduling (with type inheritance) */}
        {tab === 1 && (
          <>
            {selectedType ? (
              <Box sx={{ mb: 2, p: 1.5, bgcolor: 'info.lighter', borderRadius: 1, border: '1px solid', borderColor: 'info.light' }}>
                <Typography variant="body2" color="info.dark">
                  Showing defaults from resource type <strong>{selectedType.code} — {selectedType.name}</strong>.
                  Leave a field empty to inherit the type's default. Fill it in to override for this specific resource.
                </Typography>
              </Box>
            ) : (
        ...
        {/* Tab 2: Custom Fields (edit mode only) */}
        {tab === 2 && !isNew && initial && (
          <CustomFieldsEditor level="resource" levelId={initial.id} canManage={canManage} />
        )}
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose}>Close</Button>
...
export default function ResourceTypes() {
  ...
  async function handleDeleteResourceType() {
    ...
    setRtDeleting(true);
    try {
      await fabMutate('fabErpResourceType', 'delete', { id: rtDelete.id });
      setRtDelete(null); fetchAll();
    } catch (e: unknown) {
      const err = e as { response?: { data?: { error?: string } }; message?: string };
  ...
  async function handleDeleteResource() {
    ...
    setResDeleting(true);
    try {
      await fabMutate('fabErpResource', 'delete', { id: resDelete.id });
      setResDelete(null); fetchAll();
    } catch (e: unknown) {
      const err = e as { response?: { data?: { error?: string } }; message?: string };
      setError(err.response?.data?.error ?? err.message ?? 'Delete failed');
      setResDelete(null);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1300, mx: 'auto' }}>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" fontWeight={700}>Resource Catalog</Typography>
        <Typography variant="body2" color="text.secondary">
          Resource types with SAP-standard capacity/scheduling/costing defaults · individual resources with overrides · custom fields
        </Typography>
      </Box>
      ...
      <Stack direction="row" spacing={1} sx={{ mb: 3 }}>
        <Chip label={`${resourceTypes.length} Resource Types`} variant="outlined" />
        <Chip label={`${resources.length} Resources`} variant="outlined" />
      </Stack>
          ...
          {tab === 0 && (
            <>
              {canManage && (
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
                  <Button variant="contained" startIcon={<AddIcon />}
                    onClick={() => setRtDetail({ open: true, item: null })}>
                    New Resource Type
                  </Button>
                </Box>
              )}
              ...
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ '& th': { fontWeight: 700 } }}>
                    ...
                    <TableCell>Capacity (hrs/day)</TableCell>
                    <TableCell>Cost/hr</TableCell>
                    <TableCell sx={{ width: 96 }}>Actions</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {resourceTypes.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} align="center" sx={{ py: 4, color: 'text.secondary' }}>
                        No resource types defined. Click "New Resource Type" to add one.
                      </TableCell>
                    </TableRow>
                  ) : resourceTypes.map((rt) => {
                    const plant = rt.plantId != null ? plantMap.get(rt.plantId) : undefined;
                    return (
                      <TableRow key={rt.id} hover sx={{ cursor: 'pointer' }}
                        onClick={() => setRtDetail({ open: true, item: rt })}>
                        <TableCell><Chip label={rt.code} size="small" variant="outlined" /></TableCell>
                        <TableCell>
                          <Typography variant="body2" color="primary" sx={{ fontWeight: 500 }}>{rt.name}</Typography>
                        </TableCell>
                        <TableCell>{rt.category ?? <Typography variant="body2" color="text.disabled">—</Typography>}</TableCell>
                        <TableCell>
                          {plant
                            ? <Chip label={`${plant.code} — ${plant.name}`} size="small" />
                            : <Typography variant="body2" color="text.secondary">Company-wide</Typography>}
                        </TableCell>
                        <TableCell>
                          <Typography variant="body2">
                            {rt.capacityHrsPerDay != null ? `${rt.capacityHrsPerDay} hrs × ${rt.numUnits ?? 1} units` : <span style={{ color: '#bbb' }}>—</span>}
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography variant="body2">
                            {rt.costPerHour != null ? `${rt.costPerHour} ${rt.currency ?? 'INR'}/hr` : <span style={{ color: '#bbb' }}>—</span>}
                          </Typography>
                        </TableCell>
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <Tooltip title="Delete">
                            <span>
                              <IconButton size="small" color="error" disabled={!canManage} onClick={() => setRtDelete(rt)}>
                                <DeleteIcon fontSize="small" />
                              </IconButton>
                            </span>
                          </Tooltip>
              ...
              <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
                Click a row to view/edit details, custom fields, and formula properties.
              </Typography>
            </>
          )}
          ...
          {tab === 1 && (
            <>
              {canManage && (
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
                  <Button variant="contained" startIcon={<AddIcon />}
                    onClick={() => setResDetail({ open: true, item: null })}>
                    New Resource
                  </Button>
                </Box>
              )}
              ...
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ '& th': { fontWeight: 700 } }}>
                    ...
                    <TableCell>Plant</TableCell>
                    <TableCell>Capacity Override</TableCell>
                ...
                <TableBody>
                  {resources.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} align="center" sx={{ py: 4, color: 'text.secondary' }}>
                        No resources defined. Click "New Resource" to add one.
                      </TableCell>
                    </TableRow>
                  ) : resources.map((r) => {
                    ...
                    return (
                      <TableRow key={r.id} hover sx={{ cursor: 'pointer' }}
                        ...
                        <TableCell>
                          <Typography variant="body2" color="primary" sx={{ fontWeight: 500 }}>{r.name}</Typography>
                          {sl && <Typography variant="caption" color="text.secondary">{sl.code}</Typography>}
                        </TableCell>
                        <TableCell>
                          {rt ? <Chip label={`${rt.code} — ${rt.name}`} size="small" color="primary" variant="outlined" />
                              : <Typography variant="body2" color="text.disabled">—</Typography>}
                        </TableCell>
                        <TableCell>
                          {plant ? <Chip label={`${plant.code} — ${plant.name}`} size="small" />
                                 : <Typography variant="body2" color="text.secondary">Company-wide</Typography>}
                        </TableCell>
                        <TableCell>
                          {r.capacityHrsPerDay != null
                            ? <Chip size="small" color="primary" label={`${r.capacityHrsPerDay} hrs/day`} />
                            : <Typography variant="body2" color="text.disabled">Inherited</Typography>}
                        </TableCell>
                        <TableCell>
                          {r.costPerHour != null
                            ? <Chip size="small" color="primary" label={`${r.costPerHour} ${r.currency ?? 'INR'}/hr`} />
                            : <Typography variant="body2" color="text.disabled">Inherited</Typography>}
                        </TableCell>
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <Tooltip title="Delete">
                            <span>
                              <IconButton size="small" color="error" disabled={!canManage} onClick={() => setResDelete(r)}>
                                <DeleteIcon fontSize="small" />
                              </IconButton>
                            </span>
                          </Tooltip>
              ...
              <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
                Click a row to view/edit details and override capacity/scheduling/costing values. Blue chips indicate resource-specific overrides.
              </Typography>
            </>
          )}
        </>
      ...
      <ResourceTypeDetailDialog
        open={rtDetail.open}
        initial={rtDetail.item}
        plants={plants}
        canManage={canManage}
        onClose={() => setRtDetail({ open: false, item: null })}
        onSaved={onSaved}
      />

      <ResourceDetailDialog
        open={resDetail.open}
        initial={resDetail.item}
        resourceTypes={resourceTypes}
        plants={plants}
        canManage={canManage}
        onClose={() => setResDetail({ open: false, item: null })}
        onSaved={onSaved}
      />

      <DeleteDialog
        open={!!rtDelete}
        label={rtDelete ? `${rtDelete.code} — ${rtDelete.name}` : ''}
        onClose={() => setRtDelete(null)}
      ...
      <DeleteDialog
        open={!!resDelete}
        label={resDelete ? `${resDelete.code} — ${resDelete.name}` : ''}
        onClose={() => setResDelete(null)}
