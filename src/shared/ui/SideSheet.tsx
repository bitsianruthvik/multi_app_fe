...
/**
 * Right-docked overlay panel — the host for a record peek, and for "edit
 * without leaving the list".
 *
 * The point of a sheet over a dialog is that the reader keeps their place: the
 * list behind stays scrolled where it was, so peeking at six records in a row
 * costs six clicks instead of six navigations and six back-buttons. `onExpand`
 * is the escape hatch to the full record when the peek isn't enough.
 *
 * The body is solid (never glass — §5.3); only the scrim blurs, inherited from
 * the MuiBackdrop override in the platform theme.
 */
export function SideSheet({
  ...
  title: ReactNode;
  subtitle?: ReactNode;
  /** Shown as an "expand to the full record" affordance in the header. */
  onExpand?: () => void;
  expandLabel?: string;
