...
/**
 * cf_erp's MUI theme — the same values as fab_erp's (apps/fab_erp/theme.ts),
 * kept identical on purpose so MUI parts (buttons, fields, dialogs, tables,
 * tooltips, scrims) render the same in both apps. Only the tokens in
 * src/theme/tokens.css are shared; this file is a copy, not an import, so the
 * apps stay independent. Change both together.
 */
export function createCfErpTheme(mode: AppThemeMode) {
  ...
  return createTheme({
    ...
    components: {
      ...
      MuiTextField: {
        // fab_erp screens pass size="small" on every field; here it is the default.
