...
/**
 * The one table in cf_erp — a port of fab_erp's DataTable, kept to the same
 * behaviour and measurements so tables look and work alike in both apps
 * (DESIGN_SYSTEM.md §4.2 / §7). The notes below are fab_erp's.
 *
...
/**
 * The table's card — or a plain box when it sits in a card already (`bare`,
 * a cf_erp addition for tables inside a SectionCard or a detail tab).
 */
function Frame({ bare, children }: { bare?: boolean; children: ReactNode }) {
  return bare ? <Box sx={{ overflow: 'hidden' }}>{children}</Box> : <Surface e={1} sx={{ overflow: 'hidden' }}>{children}</Surface>;
}
 ...
 */
const DENSITY_STORAGE_KEY = 'cf_erp:density';
...
function readStored<V>(key: string | undefined, suffix: string, fallback: V): V {
  ...
  try {
    const raw = localStorage.getItem(`cf_erp:table:${key}:${suffix}`);
...
function writeStored(key: string | undefined, suffix: string, value: unknown) {
  if (!key) return;
  try {
    localStorage.setItem(`cf_erp:table:${key}:${suffix}`, JSON.stringify(value));
  } catch {
          ...
          <Box component="table" sx={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'auto' }}>
            ...
            >
              <Box component="tr">
                ...
                {rowActions && <Box component="th" scope="col" sx={{ ...cellSx, width: '1px' }} aria-label="Actions" />}
              </Box>
            </Box>
