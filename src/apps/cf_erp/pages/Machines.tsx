  ...
  return (
    <Box>
      <PageHeader title="Machines" subtitle="What does the work. A machine’s type decides what it must carry — a plasma cutter its cutting speed — and operation rules turn those values into times."
        actions={canManage && <Button variant="contained" startIcon={<AddRounded />} onClick={() => setCreating(true)}>New machine</Button>} />
      ...
      <DataTable rows={rows} columns={columns} getRowId={(m) => m.id} onRowClick={open} loading={list.loading && !list.data}
        ...
        empty={<EmptyState icon={<PrecisionManufacturingRounded />} title={filtered ? 'No machine matches' : 'No machines yet'}
          hint={filtered ? 'Try another search, type or status.' : 'Add machine types under Setup › Classification (a Family with scope Machine), then the machines themselves.'}
          action={!filtered && canManage && <Button variant="contained" onClick={() => setCreating(true)}>New machine</Button>} />} />
      <MachineDialog open={creating} existing={null} tree={tree.data} onClose={() => setCreating(false)}
